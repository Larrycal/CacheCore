 ...
 * table: 数据库表结构
 */
class CacheBusinessConfig {
    var validTime: TimeInterval
    var length: Int = 10
    var tableName: String
    var keyPath: String?

    init(validTime: TimeInterval, length: Int, tableName: String, keyPath: String? = nil) {
    ...
    convenience init(length: Int, tableName: String, keyPath: String? = nil) {
        self.init(validTime: defaultValidTime, length: length, tableName: tableName, keyPath: keyPath)
    }

    convenience init(tableName: String, keyPath: String? = nil) {
