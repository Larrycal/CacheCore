...
typealias FetchCachedObjectCallBack<CacheMappable> = (_ isSuccess: Bool, _ message: String?, _ data: [CacheMappable]?, _ lastFlag:
    String?) -> Void
typealias IsCompleteCallBack = (_ isSuccess: Bool) -> Void

class CacheHelper<T:TargetType> {
    var provider:MoyaProvider<T>
    var config:CacheBusinessConfig
    var target:T
    
    init(provider: MoyaProvider<T>,config:CacheBusinessConfig,target: T){
    ...
    /// fetch Data
    func fetchCachedData<M: CacheMappable>(config: CacheBusinessConfig,
                                      flag: String?,
    ...
    /// 批量或单个插入数据
    func insertCachedData<M: CacheMappable>(config: CacheBusinessConfig,
                                       values: [M],
                                       callBack: IsCompleteCallBack?) {
...
// MARK: string to data
extension String {
    func stringToDateByMillisecond() -> Date? {
